...
import { subscribeToAmbulanceLocation } from "@/lib/api";
...
export default function Track() {
  ...
  // Client-side routing estimation fallback
  const calculateETA = () => {
    if (!ambulanceData || !userLocation) return "Calculating...";
    const dist = calculateStraightLineDistance(ambulanceData.lat, ambulanceData.lng, userLocation.lat, userLocation.lng);
    return `${Math.round(dist * 2)} mins`;
  };
